...
class DATAREAD():
    ...
    # change -1 [-1, 1,2] ->[0,1,2]
    def transfer_all_data(self):
          for i in range(0, self.all_data.shape[1]):
            for idx, j in np.ndenumerate(self.all_data[:,i]):
                if self.all_data[:,i][idx] == -1:        
                    if self.minValues[i] == 0:             
                        self.all_data[:,i][idx]  = self.maxValues[i] +1
                    else : 
                        self.all_data[:,i][idx]  = 0
