   ...
   # utility delete multiple obj in a list
    ...
    def do_DOSOM(self):
        ...
        *****************************************************************************************************************************************
        ...
        cluster_num = self.som_discrete_original.m*self.som_discrete_original.n

        kmeans = KMeans(n_clusters = cluster_num, random_state = 0, n_init='auto')
        kmeans.fit(self.data_train_discrete_unnormalized)

        #self.som_discrete_original.fit(self.data_train_discrete_unnormalized)
       # weight_discrete_original = self.som_discrete_original.weights0
        #print(f"self.som_discrete_original.weights0 {self.som_discrete_original.weights0}")
        self.train_discrete_W0_predicted_label = kmeans.predict(self.data_train_discrete_unnormalized)   
      
        predicted_clusters_indexes, current_clustered_datas = self.get_indices_and_data_in_predicted_clusters(cluster_num,  self.train_discrete_W0_predicted_label,self.data_train_discrete_unnormalized)   
        #print(f" self.predicted_clusters_indexes { predicted_clusters_indexes}")
        self.getEachNeuronProbabilityOfEachFeatureValue(predicted_clusters_indexes)
       ...
       # self.do_COSOM(current_clustered_datas)
        ...
        #*** when validate needs to use current mapping(the real situation mappping) rather than the training sessino mapping
        self.test_discrete_W0_predicted_label = kmeans.predict(self.data_test_discrete_unnormalized)   
        predicted_clusters_indexes, current_clustered_datas = self.get_indices_and_data_in_predicted_clusters(cluster_num, self.test_discrete_W0_predicted_label,self.data_test_discrete_unnormalized)   
       ...
       # self.som_newEmbedding = newSom.SOM(self.som_discrete_original.weights0.shape[0] , self.som_discrete_original.weights0.shape[1],dim) 
        ...
        kmeans_new = KMeans(n_clusters = cluster_num, random_state = 0, n_init='auto')
        kmeans_new.fit(self.discrete_data_embedding)
        #self.som_newEmbedding.fit(self.discrete_data_embedding)
        #weight_newEmbedding = self.som_newEmbedding.weights0

        self.train_W_newembedding_predicted_label = kmeans_new.predict(self.discrete_data_embedding)    
        predicted_clusters_indexes, current_clustered_datas = self.get_indices_and_data_in_predicted_clusters(cluster_num, self.train_W_newembedding_predicted_label,self.discrete_data_embedding)   
        ...
        self.test_hybrid_newembedding_predicted_label = kmeans_new.predict(self.test_new_embedding) 

        predicted_clusters_transferred, current_clustered_datas_cleaned = self.get_indices_and_data_in_predicted_clusters(cluster_num,  self.test_hybrid_newembedding_predicted_label,self.test_new_embedding) 
        self.getLabelMapping( self.get_mapped_class_in_clusters(predicted_clusters_transferred,self.test_label_all) ,2)  
        transferred_predicted_label_test_W_transferred =  self.convertPredictedLabelValue( self.test_hybrid_newembedding_predicted_label,self.PLabel_to_Tlabel_Mapping_W_Discrete) 
    ...
    def do_CDOSOM(self):
       ...
       # print(f"self.training_new_embedding  {self.hybrid_embedding.shape} " )
        ...
        self.test_hybrid_newembedding_predicted_label = self.som_newEmbedding.predict(self.test_hybrid_embedding,weight_newEmbedding) 

        predicted_clusters_transferred, current_clustered_datas_cleaned = self.get_indices_and_data_in_predicted_clusters(weight_newEmbedding.shape[0],  self.test_hybrid_newembedding_predicted_label,self.test_hybrid_embedding) 
        self.getLabelMapping( self.get_mapped_class_in_clusters(predicted_clusters_transferred,self.test_label_all) ,2)  
        transferred_predicted_label_test_W_transferred =  self.convertPredictedLabelValue( self.test_hybrid_newembedding_predicted_label,self.PLabel_to_Tlabel_Mapping_W_Discrete) 
